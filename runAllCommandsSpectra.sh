...
./bin/parseFiresRateFile.exe input/keepTriggersSpectraList.txt  outDir/accept_Run$1.txt

#./bin/parseFiresRateFile.exe input/keepTriggersCoreList.txt outDir/avg_prescale_Run285517.txt
./bin/parseFiresRateFile.exe input/keepTriggersSpectraList.txt outDir/avg_prescale_Run$1.txt

#compile next script
sh compileWithRoot.sh src/triggerParseTreeHist.C
#execute
mkdir -p outParseDirSpectra
./bin/triggerParseTreeHist.exe outParseDir/ input/keepTriggersSpectraList.txt
